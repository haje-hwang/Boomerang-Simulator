...
public class BoomerangScript : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        spinScript = transform.Find("Boomerang model").GetComponent<Spin>(); 
    ...
    {
        physics_v3();
        spinScript.m_speedSpeed = m_rb.velocity.sqrMagnitude;
    }
    ...
    public void physics_v3()
    {
        ...
        */
        ...
        // m_rb.AddForce(Vector3.Cross(Physics.gravity, -transform.up) * SpinSpeed * Time.fixedDeltaTime);
        m_rb.MoveRotation(Quaternion.Euler(Vector3.down + m_rb.rotation.eulerAngles));
        m_rb.AddTorque(Vector3.Cross(Physics.gravity.normalized, transform.up) * Time.fixedDeltaTime, ForceMode.Force);
    }

    public void OnGrabEntered()
    {
        m_rb.isKinematic = true;
        isGrabbing = true;
        spinScript.ToggleSpinning(false);
    }
    ...
    public void OnGrabExited()
    {
        ...
        isGrabbing = false;
        spinScript.ToggleSpinning(true); 
    }
